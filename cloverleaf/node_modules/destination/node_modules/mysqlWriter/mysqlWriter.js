...
var moduleFunction = function(args) {
	...
	var getAllCounts = function(next) {
		//this causes a crash
		db.query("show tables;", function(err, rows, fields) {
			if (err) {
				console.log('show tables err=' + err + '\n');
			...
			for (var i = 0, len = rows.length; i < len; i++) {
				...
				db.query("select count(*) as 'msyql:" + tableName + "' from " + tableName, 
				function(err, rows, fields) {

					if (err) {
						console.log('show query err=' + err + '\n');
					}
					self.tableCounts=rows;
					next();
				});
			}
		});
	...
	var dbIterator = function(queryString, next) {
			...
			global.localEnvironment.log.debug({
				source: 'mysqlWrite.dbIterator' + '/' + tableName,
				type: 'finishWrite (callback) ',
				evidence: {
					query: queryString.substr(0, 100),
					err: err
				}
			});
	...
	var genCreateTableString = function(tableName, propertyNameArray) {
		...
		for (var i = 0, len = propertyNameArray.length; i < len; i++) {
			var propertyName = propertyNameArray[i].replace(/\./, '__').replace(/\[.*\]/, '');
	...
	var createTables = function(next) {
		...
		var bufferData = self.flatData,
			propertyNameArray = getPropertyNameList(bufferData[0]),
			createTableString = genCreateTableString(tableName, propertyNameArray);

		if (self.outputSpec.control.overwrite == 'all' || qtools.getByProperty(self.outputSpec.control.overwrite || {}, 'tableName', tableName)) {

			global.localEnvironment.log.info({
				source: qtools.employerFilename,
				type: 'inprocess',
				evidence: "drop table if exists " + tableName
			});
		...
		global.localEnvironment.log.info({
			source: 'mysqlWrite.createTables' + '/' + tableName,
			type: 'startWriteList ',
			evidence: {
				info: tableName + ' (' + sqlList.length + ' writes)'
			}
		});

		async.eachSeries(sqlList, dbIterator, function(err, result) {

			global.localEnvironment.log.debug({
				source: 'mysqlWrite.createTables',
				type: 'writeListComplete (callback) ',
				evidence: {
					err: err
				}
			});
	...
	var loadOneTable = function(tableName, recList) {
		...
		for (var i = 0, len = recList.length; i < len; i++) {
			...
			for (var j = 0, len2 = fieldNameSequence.length; j < len2; j++) {
				...
				var value = element[propertyName] ? element[propertyName].toString().replace(/'/g, "''") : '';
	...
	var loadTables = function(next) {
		...
		async.eachSeries(queryList, dbIterator, function() {
			next();
		});
	}
	...
	var getAuthParms = function() {
		var authFileName = global.localEnvironment.projectBasePath + '/config/dbAuthFiles/' + self.authParmsFile;
		try {
	...
	var closeConnection=function(next){
	...
	self.takeItAway = function(dataBuffer, fileWriteCallback) {
		self.bufferSet[self.tableName] = dataBuffer;
		self.dataBuffer = dataBuffer;


		var flatSpecs = dataBuffer.giveFlatSpecs()
		self.flatData = self.flattener.flattenToList(dataBuffer.getBufferData(), flatSpecs);
		...
		taskList.push(getAllCounts);
		taskList.push(closeConnection);
