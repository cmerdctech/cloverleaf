...
var moduleFunction = function(args) {
	...
	var runTransformation = function(element, next) {
		var transformName = element.type,
			...
			annotation = (element.parameters.label || element.parameters.fileName) + '/' + element.type;
		...
		global.localEnvironment.log.info({
			source: 'transformation.runTransformation',
		...
		transformer.execute(self.workingBufferList, function(result) {
			...
			global.localEnvironment.log.info({
				source: 'transformation.runTransformation (callback)',
	...
	async.eachSeries(this.transformSpecs, runTransformation, function() {
		self.callback(self.workingBufferList);
	});
