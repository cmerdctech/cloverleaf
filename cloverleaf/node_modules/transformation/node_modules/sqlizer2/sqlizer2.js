...
var moduleFunction = function(args) {
	...
	for (var i=0, len=self.input.length; i<len; i++){
		...
		if (!self.bufferSet[element].hasData()){
			var message="sqlizer2 says, input element: "+element+" is empty";
			throw message;
		}
	...
	var dbIterator = function(queryString, next) {
			global.localEnvironment.log.info({
				source: 'sqlizer2.js/dbIterator'+'/'+self.label,
				type: 'executeSql',
				evidence: {
	...
	var getTableData = function(taskItem, next) {
		var newBuffer = taskItem.buffer,
			...
			global.localEnvironment.log.info({
				source: 'sqlizer2.getTableData'+'/'+self.label,
				type: 'startQuery',
				evidence: {
	...
	var generateResults = function(next) {
		...
		for (var i = 0, len = self.export.length; i < len; i++) {
			...
			async.eachSeries(taskList, getTableData, function(err, result) {
console.log('\n=-=============  '+ __filename.replace(__dirname,'') +'   (at: generateResults.getTableData/completionCallback)  =========================\n');


			global.localEnvironment.log.info({
				source: 'generateResults.getTableData'+'/'+self.label,
...
console.log('\n=-=============  '+ __filename.replace(__dirname,'') +'   (at: sqlizerExiting to callback)  =========================\n');


			global.localEnvironment.log.info({
				source: 'sqlizer2.execute'+'/'+self.label,
				type: 'sqlizerExiting to callback',
