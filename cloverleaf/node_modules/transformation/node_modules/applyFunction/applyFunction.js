...
var moduleFunction = function(args) {
	...
	var extractData = function(next) {
		...
		for (var tableName in self.bufferSet) {
			...
			logList+="["+tableName+" "+bufferData.length+" records] "

		}

			global.localEnvironment.log.debug({
				source: 'applyFunction.extractData',
				type: 'finishedStep',
				evidence: "got data for applyFunction: "+logList
			});
	...
	var runMapping = function(processItem) {
		...
		if (typeof (self.workingDataList[processItem.destTableName]) == 'undefined') {
			...
			global.localEnvironment.log.debug({
				...
				type: 'inprocess',
				message:processItem.destTableName+" is undefined",
				evidence: {processItem:processItem}
			});
	...
	var executeProcesses = function(next) {
		...
		for (var i = 0, len = self.process.length; i < len; i++) {
			...
			global.localEnvironment.log.debug({
				source: 'applyFunction.executeProcesses'+'/'+processItem.type,
				type: 'inProcess',
				evidence: processItem.specs
			});
		}
	...
	var generateResults = function(next) {
		...
		for (var i = 0, len = self.export.length; i < len; i++) {
			...
			global.localEnvironment.log.debug({
				source: 'applyFunction.generateResults',
				type: 'queueItem',
				evidence: taskItem
			});
		...
		async.each(taskList, fillOutputBuffer, function(err, result) {
			...
			global.localEnvironment.log.debug({
				source: 'applyFunction.generateResults (callback)',
				type: 'finishOneProcess',
				err:err
			});
	...
	this.execute = function(inData, callback) {
		...
		if (self.config.cmdLineSwitches.verbose) {
			...
			global.localEnvironment.log.debug({
				source: 'applyFunction.execute',
				type: 'startTransformation',
				evidence:self.args
			});
